...
var setContactEmail = function(){
    ...
    target.find('form').submit(function(e){
        ...
        if (send){
            ...
            target.find('form #feed_msg').html('Enviando...');
            var dataToSend = $(this).serialize();
            if (sentData == dataToSend){
                target.find('form #feed_msg').html(alertMsg);
		...
		sentData = dataSplited.join('&');
            ...
            $.ajax({
                ...
                'success': function(resp){
                    resp = $.parseJSON(resp);
                    if (!resp.ok) {
                        target.find('form #feed_msg').html(resp.msg);
                    } else {
                        target.find('form #feed_msg').html(successMsg);
                        $('html, body').animate({scrollTop:target.position().top-20}, 'slow');
                    }
                    target.find('.line.capt').html('Refrescando captcha...');
            ...
            target.find('form #feed_msg').html(warningMsg);
            $('html, body').animate({scrollTop:target.position().top-20}, 'slow');
        }
        return true;
    });
